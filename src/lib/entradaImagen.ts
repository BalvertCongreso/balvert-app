...
export async function construirImagenEntrada(datos: DatosEntrada): Promise<Buffer> {
  ...
  bloques.push(
    `<text x="${anchoTarjeta / 2}" y="${yCursor}" text-anchor="middle" font-family="Arial, sans-serif" font-size="20" fill="#4b5563">${lugarTexto}</text>`
  );
  yCursor += 32;

  const fechaHoraTexto = escaparXml(recortar(`Fecha y hora: ${datos.fechaHora}`, 44));
  bloques.push(
    `<text x="${anchoTarjeta / 2}" y="${yCursor}" text-anchor="middle" font-family="Arial, sans-serif" font-size="20" fill="#4b5563">${fechaHoraTexto}</text>`
  );
  yCursor += 40;
  ...
  if (datos.empresa) {
    ...
    bloques.push(
      `<text x="${anchoTarjeta / 2}" y="${yCursor}" text-anchor="middle" font-family="Arial, sans-serif" font-size="22" fill="#374151">${empresaTexto}</text>`
    );
    yCursor += 40;
  }
  ...
  if (datos.detalleLabel && datos.detalleValor) {
    ...
    bloques.push(
      `<text x="${anchoTarjeta / 2}" y="${yCursor}" text-anchor="middle" font-family="Arial, sans-serif" font-size="22" fill="#374151">${detalleTexto}</text>`
    );
    yCursor += 40;
  }
  ...
  const svg = `
    <svg width="${anchoTarjeta}" height="${altoTarjeta}" viewBox="0 0 ${anchoTarjeta} ${altoTarjeta}" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="${anchoTarjeta / 2}" y="220" text-anchor="middle" font-family="Arial, sans-serif" font-size="30" font-weight="bold" fill="${MARRON}">${eventoEdicion}</text>
      <text x="${anchoTarjeta / 2}" y="270" text-anchor="middle" font-family="Arial, sans-serif" font-size="26" fill="${AZUL}" font-weight="600">${nombre}</text>
      ${bloques.join("\n")}
      <image x="${anchoTarjeta / 2 - 170}" y="${qrY}" width="340" height="340" href="data:image/png;base64,${qrBase64}" />
      <text x="24" y="${altoTarjeta - 24}" font-family="Arial, sans-serif" font-size="13" fill="#9ca3af">Desarrollado por AtrapaEsaRed</text>
    </svg>
  `;
